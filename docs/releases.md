...
**パッチリリース** — 依存関係の更新のみ (機能・仕様の変更なし)。オープン中の Dependabot PR 5 件 (#446〜#450) を適用。CI 全テスト pass を確認済み。

### 依存関係の更新 (PR #446〜#450)

- **Python 依存** (requirements.txt):
  - SQLAlchemy 2.0.51 → 2.0.52 / alembic 1.18.5 → 1.19.1 / python-dotenv 1.2.2 → 1.2.3 (PR #446)
  - webauthn >=2.8.0,<3.0 → >=3.0.0,<4.0 (**メジャー更新**: py_webauthn v3.0.0 — PQC (ML-DSA) 署名検証対応、EDDSA 鍵優先化、CBOR 重複キー拒否。アプリ側コード変更は不要) (PR #447)
  - boto3 >=1.43.33 → >=1.43.75 (PR #448)
  - Pillow >=12.2.0 → >=12.3.0 (PR #449)
  - cryptography >=49.0.0 → >=50.0.0 (pkcs7 復号の Bleichenbacher oracle 修正を含むが本アプリの使用機能には非該当) (PR #450)
...
- TOTP 2FA を段階導入 (PR1: secret 保管・サービス層・設定 UI (opt-in) → PR2: ログイン時の 2FA ゲート → PR3: パスキー登録時の TOTP 必須化 + 移行バナー)
- **パスキー専用ログインモード・非常用リカバリコードを撤去** (PR5) — パスキー紛失時の復旧は TOTP が担い、全ユーザーがパスワードを持つため誰もロックアウトされない。`passkey_only_login` / `recovery_code_*` カラムを物理 DROP (マイグレーション 047)
- Discord 互換 Webhook 対応 — 通知 URL を discord.com に限定せず、Discord 互換の自家ホスト Webhook も利用可能に (SSRF 対策は維持)

### 電帳法対応の全撤去

自家ホスト運用では電子帳簿保存法の真実性要件を満たせないと判断し、v4.0.0-beta で導入した電帳法対応関連機能を撤去:

- 証憑ハッシュ検証機能 (SHA-256 改ざん検知・verify エンドポイント)
- AI コンプライアンスチェック機能 (スキャナ保存要件の AI 判定)
- 証憑の入力期限警告 (受領後67日ルール)
- 訂正削除証跡 (`VoucherAuditLog`) — 論理削除を物理削除に戻す
- ドキュメント・UI 文言から「電帳法」の謳い文句を除去し中立化

`file_hash` カラムは画像配信の ETag(304) キャッシュに引き続き利用するため温存。

### 証憑ストレージクオータの無制限設定

- `STORAGE_QUOTA_BYTES_DEFAULT` に `0` (または負数) を設定すると容量上限を撤廃できるように対応 (セルフホスト向け)

### その他

- 「監査」用語を「顧問」に統一 (ユーザー向け表示・コメント・テスト文言のみ。コード識別子 `auditor`/`audit_grants` 等は据置)
- ナビバーが中間幅 (約992〜1359px) で2行に折り返す崩れを修正
- 依存パッケージ更新 (SQLAlchemy / boto3 / cryptography / qrcode)
- `claude-review.yml` の CI 設定を修正 (レビューコメントが権限エラーで投稿されない silent 障害の解消・Dependabot PR の恒常的な偽 fail チェックの解消)

### 技術詳細

- マイグレーション 046 (TOTP 関連カラム追加) / 047 (パスキー専用モード・リカバリコード関連カラムの物理 DROP、不可逆)
- pytest 1899+ 件
