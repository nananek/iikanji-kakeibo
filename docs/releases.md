...
**CSV 照合 UX 改善 (PR2): カード会社未達ハイライトと日計内訳**
...
- **journal_only の経過日数ソート** — 古い未達ほど目立つよう降順ソート
- `Reconciliation.journal_only` に `days_since_journal` / `is_stale` フィールド、`daily_summary` に `pending_card_amount` フィールド追加（API レスポンスの破壊的変更なし）
- pytest 1541 件、サーバー全体カバレッジ維持

## v3.9.0

**CSV 照合 UX 改善 (PR1): 日付ズレ照合と段階バッジ**
