...
- **暗号化** — 仕訳の摘要・日付・取引内容・**借方/貸方金額および科目コード**、医療費明細 (金額・氏名・病院名等)、証憑画像、AI 下書き画像、残高キャッシュ、バックアップ
- **非暗号化 (サーバーから見える)** — メールアドレス、勘定科目マスタ (コード・名称・区分)、AI 下書きの要約テキスト

### 鍵管理 — ログインパスワードが常用鍵 ([#385](https://github.com/nananek/iikanji-kakeibo/issues/385))

- **ログインパスワード = MK の常用解錠鍵**。`master = Argon2id(password, salt)` を HKDF split し、
  `login_verifier` (サーバ照合) と `mk_wrap_key` (MK 解錠) に分離。サーバにはパスワード生値も MK も
  渡らない (受動的運営者は MK を導けない)。**ログイン 1 回入力で MK まで解錠**
- v4 ユーザーは**初回ログイン時に透過移行** (パスワード 1 回入力 → 進捗バー → 完了)
- MK ラップ要素: **ログインパスワード** (常用) / **リカバリシード** (BIP-39 24 単語、緊急 + パスワード
  リセット) / **Passkey** (任意の上乗せ)。サーバーは MK を保持せず、すべて失うと復元不能
- **パスワードリセット**: リカバリシードで新パスワードを設定 + シードをローテーション (§3.4)
- **TOTP 2 要素認証** (任意): 認証アプリの 6 桁コードでログインを保護。Passkey 設定済みなら任意 (§3.6)
- **パスキー専用モード (`passkey_only_login`) は廃止**。全アカウントでパスワード必須化
...
仕訳明細の金額・科目コードをサーバー応答から段階的に除去している ([#338](https://github.com/nananek/iikanji-kakeibo/issues/338))。**過去に決算月3を確定済みの環境では、この除去を含むバージョンをデプロイする前に、各利用者が `設定 → 月次確定` の「すべて再暗号化」を実行**して旧形式の損益振替仕訳 (サーバー生成・未暗号化) を暗号化版へ移行する必要がある (未移行のまま除去版を配ると、確定済み年度の損益振替の金額が表示できなくなる)。`client-py` の**応答復号 (読み取り)** は新旧サーバー両対応のため、この点ではサーバーより先に配布してよい。

#### 仕訳明細の平文 write/wire 停止 (#338 item5) のデプロイ順序

仕訳明細の平文 `account_code` / `debit_amount` / `credit_amount` は、サーバーが**書かず・要求せず・検証しない** (write 停止) ようになり、クライアント (ブラウザ / `client-py`) も**送信しなくなった** (wire 停止)。貸借一致・科目存在の検査はクライアント + 監査時検査の責務へ移行している。**デプロイ順序が重要**:

1. **先にサーバー** (write 停止版 = 平文列が NULL 許容かつ平文を無視する) をデプロイする。
2. **その後にクライアント** (`client-py` の wire 停止版) を配布する。

理由: wire 停止版クライアントは line の平文を送らないため、平文列が `NOT NULL` の**古いサーバー**へ POST すると仕訳起票が失敗する。**`client-py` の wire 停止版 (送信側) は、上記の読み取り後方互換とは異なり、サーバーより先に配布してはならない。** ブラウザ版は同一サーバーイメージに同梱されるため順序問題は生じない。

#### 仕訳明細の平文列 物理 DROP (#338 item8 / migration 068) — 真の「DB 平文ゼロ」達成

仕訳明細の平文 `account_code` / `debit_amount` / `credit_amount` の 3 列と複合 FK `fk_jel_account` を migration `068_e3f_jel_drop_plaintext` で**物理削除**した。行の科目・金額は `encrypted_blob` にのみ収録され、サーバーは仕訳の金額・科目コードを一切保持・参照しない。残高集計・損益振替・試算表/P/L/B/S はすべてクライアント側で復号して計算し、貸借一致・科目存在の検査もクライアント (復号時) + 監査時検査の責務へ全面委譲した。これにより脅威モデル §1/§4 の「仕訳の金額・科目コードを守る」が実装と一致し、Epic #84 の「**DB 平文ゼロ**」が真に達成される。

**本番適用の前提 (不可逆)**:

- migration 068 は **upgrade 冒頭で旧 closing 未移行 (損益振替仕訳で `is_closing` かつ `encrypted_blob` が空) を検出すると `RuntimeError` で中断**する。事前に各利用者が `設定 → 月次確定` の「すべて再暗号化」を実行して旧 closing を暗号化版へ移行しておくこと (未移行のまま DROP すると確定済み年度の損益振替金額が復元不能になる)。
- DROP は不可逆 (downgrade は 3 列を nullable で復元するのみで値は戻らない)。**十分なバックアップ取得後に適用**すること。
- **監査時検査 (§12.11) は実装済**。平文 line 列の DROP でサーバから複式簿記の整合性検証が消えた後も、auditor が受信スナップショットを復号した時点で貸借一致・科目存在・手動損益振替をクライアント側で再検査して可視化する (`audit_review_renderer.mjs`、Lv 別粒度)。owner 書込時の `entries_builder` 検査と合わせ「クライアント側 + 監査時」の二段構えで不正入力を検出する。**本番 DROP 適用の前提条件は満たされている**。
