...
> **NOTE (2026-06)**: 鍵管理アーキテクチャは `login-derived-mk.md` で改訂中
> (**ログインパスワード主 + Passkey 任意上乗せ + リカバリシード緊急用**へ)。
> 下表「鍵管理の主軸」および §2 / §10 は **PR-3 で更新予定**。それまでは
> `login-derived-mk.md` が優先。
...
|---|---|---|
| `debit_amount + credit_amount` 合計一致 | サーバから見えず | クライアント側 + 監査時の整合性検査 (実装済、本節「監査時検査の実装」参照) |
...
上表「クライアント側 + 監査時の整合性検査」のうち **監査時検査は実装済**
(`app/static/js/audit/audit_review_renderer.mjs`)。auditor が受信スナップショットを
HPKE 復号した時点で、サーバが失った整合性検証をクライアント側で再実行し、違反を
**非ブロッキングで可視化**する (監査は異常発見が目的なので処理は止めない)。

- `validateEntryIntegrity(entry, accountsMeta, {level})` — 仕訳単位の検査:
  - **Lv3 (本人同等)**: 可視行の科目存在 / 借方・貸方の XOR / 非負整数 +
    **仕訳単位の貸借一致** + 手動 `fiscal_period=16` (is_closing でない損益振替) 検出。
  - **Lv2 (税務科目)**: 非公開科目行がマスク除外され仕訳単位の貸借が構造的に崩れる
    ため **貸借一致はスキップ**。可視行の科目存在 / XOR / 整数 + 手動 16 のみ。
  - **Lv1 (集計のみ)**: 仕訳本体が無いので `validateTrialBalance` (試算表の借方=貸方)
    で代替。Lv2 は試算表もマスクで崩れるため検査しない。
- `collectSnapshotIssues(snapshot)` がスナップショット全体を集約し、サマリ banner +
  試算表不一致の赤ハイライト + 該当仕訳の警告行として表示する。
- 併せて `snapshot_hash` (§14.3) を復号直後に `SHA-256(平文) == pkg.snapshot_hash` で
  照合する (HPKE/AEAD の defense-in-depth、非ブロッキング)。
- owner 書込時の検査は `entries_builder.buildJournalEntry` (貸借一致 / XOR / 科目非空)
  で別途担保される。両者で「クライアント側 + 監査時」の二段構えとなる。

これにより、平文 line 列の物理 DROP (#338 item8) でサーバ検証が消えた後も、不正入力
(貸借不一致・科目不在・手動損益振替) を監査時に検出できる。
