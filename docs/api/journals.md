...
> **注 (#338 item5 / v5 E2EE):** 仕訳本体・明細の実値（日付・摘要・科目コード・
> 借方/貸方金額）はクライアント側で AES-GCM 暗号化され `encrypted_blob` として
> 送信されます。サーバは平文金額・科目を保持しないため、**貸借一致・科目存在の
> 検証はサーバでは行いません**（クライアント + 監査時検査の責務）。サーバが受け取る
> 平文メタは `fiscal_year` / `fiscal_month` のみです。下記の平文フィールド例は
> E2EE 化以前の旧仕様であり、現行 wire 形式は別途 v5 E2EE ドキュメントを参照して
> ください。
...
| 400 | `entry: encrypted_blob と blob_iv は必須です` | 暗号化本体が未指定（平文-only POST 拒否） |
...
> #338 item5 で **貸借不一致・科目不存在によるサーバ拒否は撤去**されました
> （サーバが平文金額・科目を持たないため検証不能。クライアント + 監査時検査で担保）。
