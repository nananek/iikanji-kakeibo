...
> **🔒 v5.0 で End-to-End 暗号化 (E2EE) に移行します。** 仕訳の摘要・取引内容・証憑画像・医療費明細などの暗号化／復号はすべて利用者の端末で行われ、サーバーは平文を持ちません (仕訳明細の金額・科目コードは残高集計のため平文メタとして残ります)。これに伴い MCP サーバー・Webhook 通知・自家ホスト LLM・自動取込・リアルタイム代理閲覧は廃止されます。詳細と利用者がすべきことは **[v4.x → v5.0 移行ガイド](https://nananek.github.io/iikanji-kakeibo/v5-e2ee/migration.html)** を参照してください。
