...
> **🔒 v5.0 で End-to-End 暗号化 (E2EE) に移行します。** 仕訳の摘要・取引内容・**金額・科目コード**・証憑画像・医療費明細などの暗号化／復号はすべて利用者の端末で行われ、サーバーは平文を一切持ちません (残高集計・損益振替・試算表/P/L/B/S もすべて端末側で計算)。これに伴い MCP サーバー・Webhook 通知・自家ホスト LLM・自動取込・リアルタイム代理閲覧は廃止されます。詳細と利用者がすべきことは **[v4.x → v5.0 移行ガイド](https://nananek.github.io/iikanji-kakeibo/v5-e2ee/migration.html)** を参照してください。
