...
> **🔒 v5.0 で End-to-End 暗号化 (E2EE) に移行します。** サーバーは仕訳・証憑画像・医療費などの平文を一切持たなくなり、暗号化／復号はすべて利用者の端末で行われます。これに伴い MCP サーバー・Webhook 通知・自家ホスト LLM・自動取込・リアルタイム代理閲覧は廃止されます。詳細と利用者がすべきことは **[v4.x → v5.0 移行ガイド](https://nananek.github.io/iikanji-kakeibo/v5-e2ee/migration.html)** を参照してください。
