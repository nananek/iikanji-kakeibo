...
確認画面の共通機能:
- **科目自動推定** — 過去の仕訳履歴から摘要に応じた費目を自動提案。AI科目推定ボタンで未設定行を一括AI推定。チェックした行は設定済みでも上書き推定可能
...
受診者・病院名・治療内容・支払額・保険補填額を記録。年間の自己負担額を自動集計し、確定申告の医療費控除に対応します。

### 月次確定・年度管理

月ごとに帳簿を「確定」して編集をロック。確定済み期間の仕訳は追加・変更・削除が不可になり、帳簿の整合性を保護します。確定済み月の残高をキャッシュし、試算表・元帳の表示を高速化。年末には損益振替仕訳（専用の「振替」期間）を自動生成し、収益・費用を繰越利益に振替えます。

前々年以前の年度は明示的に「開設」しない限り仕訳を打てない安全設計。翌年に確定済み期間がある場合は前年の確定解除をブロックし、年度間の整合性を保護します。インポート時に古い年度の取引が含まれる場合は、無視するか当年1/1付の元入金仕訳に変換するかを選択できます。
...
| 残高試算表 | 全勘定科目の借方・貸方・残高一覧。期間バーでドラッグ選択、当期純利益・負債純資産合計で貸借一致チェック |
| 総勘定元帳 | 科目別の取引履歴と残高推移。月区切り・期間絞込・モーダル編集・削除対応 |
...
iPhone等のスマートフォン縦長画面に最適化されたレスポンシブレイアウト:

- **列の自動最適化** — 画面幅576px以下で低優先度の列（科目コード・伝票番号・相手科目等）を自動非表示
- **月別残高表示** — 元帳のモバイル表示では残高列の代わりに月ごとのサマリー行で残高を表示
- **期間バー縮小** — 16ボタンの期間選択バーをモバイル幅に収まるよう自動縮小
- **タッチ操作対応** — ホバー効果の残留を防止し、チェックボックス列のみドラッグ選択を有効化

### PWA 対応

ホーム画面に追加してネイティブアプリのように利用可能。Service Worker によるキャッシュでオフライン表示にも対応。明示的にログアウトしない限りセッションが維持されます。
